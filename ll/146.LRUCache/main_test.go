...
func TestLRUCache(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			lru := Constructor(tt.input[0][0])
			for _, item := range tt.input[1:] {
				if len(item) == 1 {
					lru.Get(item[0])
				} else {
...
func (lru *LRUCache) Get(key int) int {
	if node, ok := lru.items[key]; ok {
		lru.dll.PushNode(node)
		return node.data.(cacheItem).value
	}
	return -1
}
...
func (lru *LRUCache) Put(key int, value int) {
	...
	if len(lru.items) == lru.cap {
		if node := lru.dll.Tail(); node != nil {
			delete(lru.items, node.data.(cacheItem).key)
			lru.dll.Delete(node)
		}
	}

	lru.items[key] = lru.dll.Push(cacheItem{key, value})
}

type LRUCacheSlow struct {
	cap        int
	agesToKeys []int
	items      map[int]*item
}
...
func (lru *LRUCacheSlow) Get(key int) int {
	if v, ok := lru.items[key]; ok {
		age := len(lru.agesToKeys) - 1
		if v.age == age {
			return v.value
		}

		lru.agesToKeys = append(lru.agesToKeys[:v.age], lru.agesToKeys[v.age+1:]...)
		lru.updateAges(lru.agesToKeys[v.age:], v.age)

		v.age = age
		lru.agesToKeys = append(lru.agesToKeys, key)
...
func (lru *LRUCacheSlow) Put(key int, value int) {
	age := len(lru.items)
	if v, ok := lru.items[key]; ok {
		age--
		if v.age == age {
	...
	if age == lru.cap && age > 0 {
		// delete lru key and shift all existing keys
		lruKey := lru.agesToKeys[0]
		lru.agesToKeys = lru.agesToKeys[1:]
		delete(lru.items, lruKey)

		lru.updateAges(lru.agesToKeys, 0)
	}

	// append new key
	lru.items[key] = &item{value, len(lru.items)}
	lru.agesToKeys = append(lru.agesToKeys, key)
}

// updateAges decrements age for each shifted key
func (lru *LRUCacheSlow) updateAges(agesToKeys []int, startAge int) {
	for i, key := range agesToKeys {
		lru.items[key].age = startAge + i
	}
}
